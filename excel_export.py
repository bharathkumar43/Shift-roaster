...
SHIFT_COLORS = {
    1: PatternFill(start_color="D5F5E3", end_color="D5F5E3", fill_type="solid"),  # light green
    2: PatternFill(start_color="D6EAF8", end_color="D6EAF8", fill_type="solid"),  # light blue
    3: PatternFill(start_color="F9E79F", end_color="F9E79F", fill_type="solid"),  # light yellow
}
...
def generate_excel(roster, warnings, shift_assignments, employees, year, month):
    """Generate a formatted Excel workbook for the monthly roster."""
    wb = Workbook()
    ...
    if warnings:
        _create_warnings_sheet(wb, warnings)
...
def _create_roster_sheet(wb, roster, shift_assignments, employees, year, month):
    ...
    for shift_num in [1, 2, 3]:
        ...
        for emp in shift_emps:
            label = f"{emp['name']} [{emp['content_type']}]"
...
def _create_summary_sheet(wb, shift_assignments, employees):
    ...
    headers = ["Employee", "Content Type", "Working Days", "Assigned Shift", "Shift Timing"]
    ...
    for shift_num in [1, 2, 3]:
        ...
        for emp in shift_emps:
            ...
            ws.cell(row=row, column=2, value=emp["content_type"]).border = THIN_BORDER
            ws.cell(row=row, column=3, value=", ".join(emp["working_days"])).border = THIN_BORDER
